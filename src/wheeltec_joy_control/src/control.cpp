...
wheeltec_joy::wheeltec_joy() 
{
  ...
  //  pub = n.advertise<geometry_msgs::Twist>("cmd_vel",1);//将速度发给机器人底盘节点
   pub = n.advertise<geometry_msgs::Twist>("rear_agv_vel",1);
...
void wheeltec_joy::callback(const sensor_msgs::Joy::ConstPtr& Joy) //键值回调函数
 {
   double vangle_key,linera_key,angle_key,raise_key;
   double acce_x,acce_z;
   geometry_msgs::Twist v;
   angle_key =Joy->axes[axis_ang];  //获取axes[0]的值
   vangle_key =Joy->axes[scale_angular];
   linera_key =Joy->axes[axis_lin];  //获取axes[1]的值
   raise_key =Joy->axes[7];  //获取axes[7]的值
  ...
  //  }
   ...
   //打印输出
//    ROS_INFO("linear:%.3lf angular:%.3lf,adjust:%.3lf",vlinear_x.data,v.angular.z,v.angular.x);
    // ROS_INFO("raise car statue:%.3lf",v.angular.y);
   pub.publish(v);
 
}
int main(int argc,char** argv)
{
  ros::init(argc, argv, "rear_joy_control");
